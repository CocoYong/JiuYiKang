...
//微信开放平台注册的appid
#define WEIXINAPPID @"wx32db1f85a151bbdf"
//微信开放平台注册的商户ID
#define MERCHANT_ID @"1489154332"
//微信开放平台注id生产的密钥
#define MERCHANT_PRIVAT_SECRET @"dbe7a33f58d304d1927927835baef145"
...
-(void)payForWeiXin:(NSDictionary*)pramasDic andDelegateController:(UIViewController*)viewController andCallBackBlock:(PayBackBlock)callBackBlock
{
    ...
    {
        NSMutableString *stamp  = [pramasDic objectForKey:@"timestamp"];
        
        //调起微信支付
        PayReq* req             = [[PayReq alloc] init];
        req.partnerId           = [pramasDic objectForKey:@"partnerid"];
        req.prepayId            = [pramasDic objectForKey:@"prepayid"];
        req.nonceStr            = [pramasDic objectForKey:@"noncestr"];
        req.timeStamp           = stamp.intValue;
        req.package             = [pramasDic objectForKey:@"package"];
        req.sign                = [pramasDic objectForKey:@"sign"];
        [WXApi sendReq:req];
        _payBlock=callBackBlock;
        NSLog(@"request===%@",req);
    }
}
...
-(void)payForWeixinWith:(NSMutableDictionary*)paramsDict andCallBackBlock:(PayBackBlock)callBackBlock
{
    ...
    if (paramsDict!=nil) {
       ...
       NSString *randomString=[self md5:[NSString stringWithFormat:@"%d",arc4random()% 10]];
        ...
        NSMutableString *trade_noSting=[[NSMutableString alloc]initWithString:dateString];
        [trade_noSting appendString:randomString];
        [paramsDict setObject:trade_noSting forKey:@"out_trade_no"];
        [paramsDict setObject:[self getIPAddress] forKey:@"spbill_create_ip"];
